...
router.post('/', async (req, res) => {
  const { nome, emoji, preco, custo, estoque, estoque_minimo, categoria, combo_qtd, combo_preco, dose_ml, garrafa_ml, garrafa_preco } = req.body;
  if (!nome)
    return res.status(400).json({ error: 'Nome é obrigatório' });
  try {
    const [result] = await db.query(
      'INSERT INTO produtos (nome, emoji, preco, custo, estoque, estoque_minimo, evento_id, categoria, combo_qtd, combo_preco, dose_ml, garrafa_ml, garrafa_preco) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [nome, emoji || '🍺', preco || 0, custo || 0, estoque || 0, estoque_minimo || 5, req.eventoId, categoria || '', combo_qtd || null, combo_preco || null, dose_ml || null, garrafa_ml || null, garrafa_preco || null]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!chk.length) return res.status(404).json({ error: 'Produto não encontrado' });
    await db.query(
      'UPDATE produtos SET nome=?, emoji=?, preco=?, custo=?, estoque=?, estoque_minimo=?, categoria=?, combo_qtd=?, combo_preco=?, dose_ml=?, garrafa_ml=?, garrafa_preco=? WHERE id=?',
      [nome, emoji, preco, custo || 0, estoque, estoque_minimo, categoria || '', combo_qtd || null, combo_preco || null, dose_ml || null, garrafa_ml || null, garrafa_preco || null, req.params.id]
    );
    const [rows] = await db.query('SELECT * FROM produtos WHERE id = ?', [req.params.id]);
